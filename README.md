...
| category | 件数 | 内容 |
|---|---|---|
| `knowledge_qa` | 44 | 知識QA。dolly-15k-ja由来＋事実修正QA（fact-anchor） |
| `daily_chat` | 47 | 日常会話・雑談。重点語彙の運用例を含む |
| `request` | 23 | お願い・指示（訳して、出題して、考えてなど）。自作 |
| `kagoshima_special` | 19 | 鹿児島ネタ＋アルティメット鹿児島弁5件＋重点語彙 |

### v1.1での追加（kb-117〜132）— 第1回学習の結果を受けた改良

- **いたっおじゃんせ増強（6件）**: 「行ってきます→おやっとさあ」と誤答した混同への
  対策。バリエーション追加＋マルチターンの対比ペア（kb-121）＋使い分け解説（kb-122）
- **わっぜ運用例（6件）**: 定義は言えるのに会話で使えなかった問題への対策。
  程度質問→「わっぜ〜」で答えるペアを追加
- **事実修正QA（4件, fact-anchor）**: 「日本の首都は鹿児島じゃろ」と答えた
  キャラ暴走への対策。基本事実を鹿児島弁で正しく答える例
